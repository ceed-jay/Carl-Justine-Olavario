...
const MenuBar: React.FC<MenuBarProps> = ({ sections, activeSection }) => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 h-10 bg-white/80 backdrop-blur-lg border-b border-gray-200/80 text-sm">
        <div className="container mx-auto px-4 flex items-center justify-between h-full">
          <div className="flex items-center space-x-4">
            ...
            <span className="font-bold">Olavario Carl Justine</span>
            <nav className="hidden md:flex items-center space-x-1">
              {sections.map(section => (
                <button 
                    key={section.id} 
                    onClick={() => handleLinkClick(section.ref)} 
                    className={`px-3 py-1 rounded-md transition-colors text-gray-800 ${activeSection === section.id ? 'bg-black/10' : 'hover:bg-black/10'}`}>
                  {section.name}
                </button>
              ))}
            </nav>
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-opacity duration-300 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        ...
        <nav className="absolute top-10 right-0 bottom-0 left-0 p-8">
          <ul className="flex flex-col items-center justify-center h-full space-y-8">
            {sections.map(section => (
              <li key={section.id}>
                <button onClick={() => handleLinkClick(section.ref)} className={`text-2xl font-bold transition-colors ${activeSection === section.id ? 'text-blue-500' : 'text-gray-800'}`}>
                  {section.name}
                </button>
              </li>
            ))}
